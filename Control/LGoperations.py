...
class LGoperations:
    ...
    def eh_vazia(glc):
        ...
        if glc_ssi.get_dict_glc() == {}:
            return True, nf, vi, glc_ssi
        else:
            return False, nf, vi, glc_ssi

    @staticmethod
    def eh_finita(glc):
        glc_sem_ciclo, na = GLCPropria.remove_ciclos(glc)

        glc_sem_inuteis,  nf, vi = EliminarSimbolosInuteis.eliminar_simbolos_inuteis(glc_sem_ciclo)

        dict_glc = glc_sem_inuteis.get_dict_glc()

        if dict_glc == {}:
            return True, nf, vi, na, glc_sem_inuteis, glc_sem_ciclo
            ...
            for producoes in dict_glc[vn]:
                for simbolo in producoes:
                    if simbolo in ja_visitados:
                        return False, nf, vi, na, glc_sem_inuteis, glc_sem_ciclo
